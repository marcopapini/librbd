...
/**
 * rbdKooNBddWorkerNeon
 *
 * Generic KooN RBD Worker function exploiting BDD Evaluation with AArch64 NEON instruction set
 *
 * Input:
 ...
 *  NULL
 */
HIDDEN void *rbdKooNBddWorkerNeon(struct rbdKooNBddData *data)
{
    unsigned int time;
    unsigned int steps;
    ...
    while (time < data->numTimes) {
        ...
        if (rbdKooNBddNeon(data, data->bddmgr->root, time, steps) == NULL) {
            return NULL;
        }
 ...
 *
 * Compute the Reliability value for a BDD Node with AArch64 NEON 128bit
 *
 * Input:
 *      double *r
 *      double *h
...
HIDDEN FUNCTION_TARGET("+simd") void rbdKooNBddStepV2dNeon(double *r, double *h, double *l, double *o)
{
    ...
    v2dR = vld1q_f64(r);
    v2dL = vld1q_f64(l);
    v2dRes = vfmsq_f64(v2dL, v2dR, v2dL);
    v2dH = vld1q_f64(h);
    v2dRes = vfmaq_f64(v2dRes, v2dR, v2dH);
    vst1q_f64(o, capReliabilityV2dNeon(v2dRes));
}
 ...
 *  using AArch64 NEON instruction set
 *
 * Parameters:
 ...
 */
static FUNCTION_TARGET("+simd") double *rbdKooNBddNeon(struct rbdKooNBddData *data, int nodeIdx, unsigned int timeStart, unsigned int numSteps)
{
    double *nodeValues;
    ...
    high = rbdKooNBddNeon(data, node->high, timeStart, numSteps);
    if (high == NULL) {
        return NULL;
    }
    /* Recursively evaluate the reliability of the low part of the current BDD Node */
    low  = rbdKooNBddNeon(data, node->low, timeStart, numSteps);
    if (low == NULL) {
        return NULL;
    }
    ...
    for (tIdx = 0; (tIdx + V2D) <= numSteps; tIdx += V2D) {
        ...
        rbdKooNBddStepV2dNeon(&rel[tIdx], &high[tIdx], &low[tIdx], &nodeValues[tIdx]);
    }
    /* Is 1 time instant remaining? */
    if (tIdx < numSteps) {
        /* Compute the (cached) reliability curve associated with the current BDD Node */
